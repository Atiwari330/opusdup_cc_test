# Test CPT code analysis on uploaded transcript
param(
    [string]$TranscriptId = "d4b24576-3d9b-4bd0-959b-ca6b148263a9"
)

Write-Host "Testing CPT Code Analysis" -ForegroundColor Green
Write-Host "=========================" -ForegroundColor Green
Write-Host "Transcript ID: $TranscriptId" -ForegroundColor Cyan

try {
    $analysisBody = @{
        analysisTypes = @("cpt_codes")
    } | ConvertTo-Json
    
    Write-Host "Starting CPT code analysis..." -ForegroundColor Yellow
    ...
    Write-Host "CPT Analysis successful!" -ForegroundColor Green
